...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    ...
    Generally positive trend with a huge dip between 1640 and 1660.
...
```{r}
plot(arbuthnot$year, arbuthnot$boy/ (arbuthnot$boys+arbuthnot$girls))
```
...
These numbers are much larger (millions vs thousands).
...
Each year there are more boys born than girls. This holds true to Arbuthnot's observation.    
```{r}
