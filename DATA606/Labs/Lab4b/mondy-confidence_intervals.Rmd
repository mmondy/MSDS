...
The distribution is unimodal.It is slightly right skewed with some outliers off to the right of the graph. I would say the typical size would be around 1525. I interpreted typical to be the average size. 
...
I would not expect it to be identical, but it should be similar because you would be taking another sample from the same populations.
...
95% confidence means that 95% of the time, the confidence interval will contain the mean. 
...
My confidence interval does capture the true average size of house in Ames.
...
We would expect 95 percent of the confidence intervals to capture the true population mean. This is what 95% confidence means. 
...
45/50 = 90% of the confidence intervals include the true population mean. This is not exactly equal to the confidence interval, because there is still some variability in sampling. If we took more sample, I would think this proportion would get closer to 95%. 
...
90%, the appropriate critical value = 0.10
...
44/50 = 88% of the new intervals include the true population mean. It's relatively close to the confidence level selected.
