...
Each case represents a birth in NC. There are 1000 cases in our sample. 
...
The 1000 observations were selected randomly and represent less than 10% of the total population of births in NC. The sample size for each group is large (at least 30). Based on the boxplots above, the data is fairly symmetric with some outliers/skew, but since the sample size is large enough it can be treated as nearly normal.
...
Null Hypothesis: The is no difference between the averages weights of babies born to smoking and non-smoking mothers.
Alternative Hypothesis: There is a difference between the averages weights of babies born to smoking and non-smoking mothers.
...
The age cutoff is for younger mothers is 34 years old. Mature mothers are 35 and older. 
```{r}
...
Is the average length of pregnancy for younger mothers different than the average length of pregnancy for mature mothers?
...
Since the p-value is greater than 0.05, we fail to reject the null hypothesis. Therefore, we conclude that there is no difference in the average length of pregnancy for younger mothers compared to mature mothers. 
