...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak length of 1 means there is 1 hit and 1 miss. A streak length of 0 means there is 1 miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    The distribution of streak lengths seems to be right skewed. There is a natural boundary at 0 and the most baskets (hits) in a streak were 4. His typical streak length was around 0 to 1. 
...
The distribution is right skewed (tail trailing off to the right).The typical streak length is around 0/1. The longest streak is 5 baskets.
...
I would expect the distribution to be somewhat similar since the sample is of a decent size. 
...
After looking at the distributions through bar charts, box plots, and viewing summary statistics, I would think there is not enough evidence to show that there is much difference from the sim_basket streak (non hot hand shooter) and Kobe. I tried to run a t-test for the difference between 2 means and see that 0 is within the confidence interval so there is not enough evidence to reject the null. 
...
##First time trying this out, so I'm not sure I did this correctly.
