...
Males in sample: 9,569
```{r}
table(cdc$gender)
```

Proportion in excellent health = 0.23285
```{r}
...
Smoking habits and gender are associated or dependent because the columns are divided in different vertical locations.
...
This boxplot shows that the median BMI is fairly close, but slightly increases between the groups. 
Variability also increases from excellent to poor. 

I am choosing smoke100 because there is a stigma that smokers tend to be skinner, thus would have
lower BMI.

```{r}
boxplot(bmi ~ cdc$smoke100)
```
Surprisingly, there doesn't appear to by much difference at all between the groups. The center and spread are
about equal. 
...
They are similar in shape, but adding bins helps display more detail and to see the shape better.
...
```
There variables seem to have a positive coorelation.
...
wdiff is discrete numerical. If an observation of wdiff is 0, it means that the person is at their desired
weight, or in other word their weight = desired weight. If wdiff is positive, the person desired to gain 
weight, while if the wdiff is negative, they would like to lose weight. 
...
The center is around -10 and after removing outliers, seems fairly symmetric. The 3rd quartile is around 0, which shows that 75% of people feel that they should lose weight. 50% of people desire to lose between 0 and 21 pounds.

```{r}
hist(wdiff, breaks= 50)
boxplot(wdiff, outline=FALSE)
median(wdiff)
summary(wdiff)
```
...
Women appear to want to lose more weight than men. The median is lower and there is a larger spread.
    
```{r}
men <- subset(cdc, cdc$gender== 'm')
women <-subset(cdc, cdc$gender== 'f')

mdiff <- men$wtdesire - men$weight
wmdiff <- women$wtdesire - women$weight
...
70.76% are within one standard deviation of the mean.
    
```{r}
mean(cdc$weight)
sd(cdc$weight)

onesd <- subset(cdc, cdc$weight <= mean(cdc$weight)+sd(cdc$weight) & cdc$weight >= mean(cdc$weight)-sd(cdc$weight))
summary(onesd$weight)
nrow(onesd)/20000
```
